...
exports.validateEmail = function validateEmail(req, res, next, id) {
  ...
  if (validationErrors) {
    ...
    var tbl = 'content';
    var show_tbl = "SELECT sn, data_key, data FROM "+tbl+" WHERE sn = ?";
...
exports.create = function create(req, res, next) {
  ...
  req.checkBody('data_key').notEmpty().withMessage('Data Key is important how on earth is the system going to know what policy are you goint to set!');
  req.checkBody('data').notEmpty().withMessage('This field is mandatory must enter data!');
  var validationErrors = req.validationErrors();
  if (validationErrors) {
    res.status(400);
  ...
  var tbl = 'content'
  var sql = "INSERT INTO "+tbl+" (data_key, data) VALUES (?,?)";
  var check_sql = 'SELECT data_key, data FROM `'+tbl+'` WHERE `data_key` = ?';
  var updatQL = 'UPDATE '+tbl+' set data = ? WHERE data_key = ?';
  var ParamUpdat = [body.data, body.data_key]
  var parameters = [body.data_key, body.data];
  var chkParams = [body.data_key];
...
exports.fetchAll= function fetchAll(req,res,next){
  var tbl = 'content';
  var show_tbl = "SELECT sn, data_key, data FROM "+tbl+"";
...
exports.update = function update(req, res, next){
  const sn = req.doc.sn;
    var body = req.body;
    var tbl = 'content';
    var show_tbl = "UPDATE "+tbl+" set data_key = ?, data = ? WHERE sn = ?";
    config.MySQL.query(show_tbl, [body.data_key, body.data, sn], function (err, result) {
      if (err) throw err;
